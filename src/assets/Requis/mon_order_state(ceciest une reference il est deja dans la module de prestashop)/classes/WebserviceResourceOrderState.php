...
class WebserviceSpecificManagementOrderStateUpdate implements WebserviceSpecificManagementInterface
{
    ...
    const STATE_PASS_PARTOUT   = 8;   //  (pass-partout) (a)
     ...
     *
     * STATE_PASS_PARTOUT (8) : toutes transitions autorisées → géré séparément.
    ...
    protected static $allowedTransitions = [
        ...
        // STATE_PASS_PARTOUT : autorisé vers tout, testé dans validateTransition()
    ...
    protected function validateTransition(int $currentState, int $targetState)
    {
        ...
        if ($currentState === self::STATE_PASS_PARTOUT || $currentState === 11) {
            return;
        }
    ...
    protected function managePost()
    {
        ...
        if ($idOrderState === self::STATE_DELIVERED) {
            $this->insertStockMovements($order->getProducts());
        }
     ...
     */
    protected function insertStockMovements(array $products)
    {
        $db  = Db::getInstance();
